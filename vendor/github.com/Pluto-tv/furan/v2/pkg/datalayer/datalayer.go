...
import (
	...
	"github.com/Pluto-tv/furan/v2/pkg/models"
...
func NewRawPGClient(pguri string, maxconns uint32) (*pgxpool.Pool, error) {
	...
	dbcfg.MinConns = int32(MinPoolConns)
	...
	dbcfg.AfterConnect = func(ctx context.Context, conn *pgx.Conn) error {
		conn.TypeMap().RegisterType(&pgtypeV5.Type{
			Name:  "uuid",
			OID:   pgtypeV5.UUIDOID,
			Codec: pgtypeV5.TextCodec{},
		})
		return nil
	}
...
func (dl *PostgresDBLayer) CreateBuild(ctx context.Context, b *models.Build) (uuid.UUID, error) {
	...
	_, err = dl.p.Exec(ctx,
		...
		b.ID, b.GitHubRepo, b.GitHubRef, b.EncryptedGitHubCredential, b.ImageRepos, b.Tags, b.CommitSHATag, b.BuildOptions, b.Request, models.BuildStatusNotStarted)
	if err != nil {
		return b.ID, fmt.Errorf("error inserting build: %w", err)
	}
	return b.ID, nil
}
...
func (dl *PostgresDBLayer) GetRootAPIKey(ctx context.Context) (models.APIKey, error) {
	out := models.APIKey{}
	err := retry(func() error {
		return dl.p.QueryRow(ctx, `SELECT id, created, name, description FROM api_keys WHERE name = 'root' AND description = 'root api key' ORDER by created ASC LIMIT 1;`).Scan(&out.ID, &out.Created, &out.Name, &out.Description)
	})
	if err != nil {
		if err == pgx.ErrNoRows {
			return out, ErrNotFound
		}
		return out, fmt.Errorf("error getting root api key: %w", err)
	}
	return out, nil
}
